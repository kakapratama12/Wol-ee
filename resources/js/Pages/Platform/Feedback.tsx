...
export default function Feedback({ feedback, filters, tenants, statuses }: Props) {
    ...
    const submitFilters = (e: FormEvent) => {
        e.preventDefault();
        router.get('/platform/feedback', filterForm.data, { preserveState: true, preserveScroll: true });
    };
    ...
    return (
        <AppLayout title="Feedback Inbox">
            ...
            <Card>
                ...
                <CardContent>
                    <form onSubmit={submitFilters} className="mb-4 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
                        <Select value={filterForm.data.status} onChange={(e) => filterForm.setData('status', e.target.value)}>
                            <option value="">Semua status</option>
                            {statuses.map((status) => <option key={status} value={status}>{status}</option>)}
                        </Select>
                        <Select value={filterForm.data.tenant_id} onChange={(e) => filterForm.setData('tenant_id', e.target.value)}>
                        ...
                        <TableBody>
                            {feedback.data.map((row) => (
                                <TableRow key={row.id}>
                                    ...
                                    <TableCell>
                                        <div>{row.tenant ?? '-'}</div>
                                        <div className="text-xs text-muted-foreground">TG {row.telegram_user_id}</div>
                                    </TableCell>
                                    <TableCell className="max-w-sm whitespace-pre-wrap">{row.feedback_text}</TableCell>
                                    <TableCell className="max-w-xs whitespace-pre-wrap text-muted-foreground">{row.original_message ?? '-'}</TableCell>
                                    <TableCell className="uppercase text-xs text-muted-foreground">{row.status}</TableCell>
                                    <TableCell>
                                        <Button type="button" variant="outline" size="sm" onClick={() => openEdit(row)}>Review</Button>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
            ...
            {editing && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="w-full max-w-lg rounded-lg bg-background p-6 shadow-lg">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">{editing.feedback_text}</p>
                        <form onSubmit={submitEdit} className="mt-4 space-y-4">
                            ...
                            <Input value={editForm.data.note} onChange={(e) => editForm.setData('note', e.target.value)} placeholder="Catatan kurasi (opsional)" />
